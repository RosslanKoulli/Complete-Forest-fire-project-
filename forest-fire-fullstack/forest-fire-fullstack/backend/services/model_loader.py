...
-------------------------
The existing project (the Streamlit version) is expected to be a sibling
or parent of this backend directory:

    forest-fire-prediction/      <- the existing project
    forest-fire-fullstack/       <- this one
        backend/
            services/model_loader.py  <- this file

If the original project lives somewhere else, set the env variable
